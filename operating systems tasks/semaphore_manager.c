...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
  ...
  int ex=get_semaphore_object_ID( ownerEnvID,semaphoreName);
  if (ex!=E_SEMAPHORE_NOT_EXISTS){
    return E_SEMAPHORE_EXISTS ;
  }
  else{
    int obj=allocate_semaphore_object(&sem);
 ...
 //Return:
 // a) SemaphoreID (its index in the array) if succeed          ---> Done <---
 // b) E_SEMAPHORE_EXISTS if the semaphore is already exists    ---> Done <---
 // c) E_NO_SEMAPHORE if the the array of semaphores is full    ---> Done <---
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
   ...
   if(index!=E_SEMAPHORE_NOT_EXISTS)
   {
     ...
     if (semaphores[index].value<0)
     {
       ...
       enqueue(&semaphores[index].env_queue, curenv);
       curenv=NULL;
       fos_scheduler();
 ...
 // Steps:
 // 1) Get the Semaphore                                                      --->Done<---
 // 2) Decrement its value                                                    --->Done<---
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
   ...
   if(index!=E_SEMAPHORE_NOT_EXISTS)
   {
     semaphores[index].value++;
     if (semaphores[index].value<=0){
        denv=dequeue(&semaphores[index].env_queue);
        sched_insert_ready(denv);
     }
   }
