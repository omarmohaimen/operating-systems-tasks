...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	struct Env* myenv = curenv; //The calling environment
		...
		uint32 *ptr_pg_tble = NULL;
		struct Share *sh=NULL;
		if(get_share_object_ID(shares[MAX_SHARES].ownerID,shares[MAX_SHARES].name)!=E_SHARED_MEM_NOT_EXISTS)
			return E_NO_SHARE;
		int index =allocate_share_object(&sh);
		if (index >= MAX_SHARES)
			return E_NO_SHARE;
		int m = allocate_chunk(myenv->env_page_directory,(uint32)virtual_address,ROUNDUP(size,PAGE_SIZE),PERM_WRITEABLE|PERM_USER);
		...
		uint32 f=0;
		for(uint32 i=(uint32)virtual_address;i<((uint32)ROUNDUP(virtual_address+size,PAGE_SIZE));i+=PAGE_SIZE)
		{
			struct FrameInfo *z =get_frame_info(myenv->env_page_directory,i,&ptr_pg_tble);
			add_frame_to_storage(shares[index].framesStorage,z,f);
			f++;
		}
		if(m==0)
		{
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	struct Env* myenv = curenv; //The calling environment
		...
		{
			int fcounter = ROUNDUP(shares[index].size,PAGE_SIZE)/PAGE_SIZE;
			for (uint32 i = 0; i<fcounter ; i++)
			{
				struct FrameInfo* frame = get_frame_from_storage(shares[index].framesStorage, i);
				int perm ;
				if (shares[index].isWritable == 1)
				perm =  PERM_USER| PERM_WRITEABLE;
				if (shares[index].isWritable == 0)
					perm =  PERM_USER;
